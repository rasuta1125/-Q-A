...
| `/api/qa/bulk-import` | Q&A一括インポート | POST | `{ items: Array<{ category, question, answer, keywords?, priority?, is_active? }> }` |
...
#### 一括インポート（NEW!）
1. **Q&A管理画面（`/admin`）で「一括インポート」ボタンをクリック**
...
1. **✅ 完了: 一括インポート機能を実装**
   - LINEや過去の問い合わせからテキストをコピー&ペーストで簡単登録
   - プレビュー機能で登録前に内容を確認可能
   - 複数件のQ&Aを一度に登録できる

2. **🔄 進行中: Q&Aデータの充実**:
   - **LINEの過去の問い合わせをエクスポート**
   - 一括インポート機能を使って初期データを投入
   - カテゴリの追加・調整
   - キーワードの最適化
